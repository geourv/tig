...
Una extracció materialitza les files que compleixen el criteri en una capa o taula nova. Quan forma part de `pr3`, la sortida s'ha de desar a `sandbox/pr3-consultes-cognom.gpkg` amb un nom intern estable, conservar l'identificador de l'entrada i registrar el criteri i el recompte; una sortida temporal no és un resultat retingut.
...
## Cas guiat sobre la instantània `pr3` amb fonts del CNIG

Amb QGIS tancat, es copia `dist/pr2-digitalitzacio-cognom.gpkg`. La còpia nova es desa com a `sandbox/pr3-consultes-cognom.gpkg`; no es copia ni es reanomena el `.qgz` de `pr2`. Des de la còpia s'obre el projecte incrustat heretat, es desa com a `pr3` i s'elimina l'entrada `pr2`. Totes les fonts locals es reorienten al GeoPackage `pr3` i es comprova que no apuntin a `dist/`, a `pr2-digitalitzacio-cognom.gpkg` ni a una ruta personal. Les famílies previstes són les [divisions administratives](https://www.idee.es/csw-codsi-idee/srv/spa/catalog.search#/metadata/spaignLLM), les [poblacions](https://www.idee.es/csw-codsi-idee/srv/spa/catalog.search#/metadata/spaign_IGR_Poblaciones) i les [xarxes de transport](https://www.idee.es/csw-codsi-idee/srv/spa/catalog.search#/metadata/spaign_IGR_Transporte) de l'IGN/CNIG. Aquests noms només identifiquen famílies de productes: no permeten anticipar quins fitxers, capes, geometries o camps contindrà una descàrrega concreta. Per a cada edició cal inspeccionar el paquet i les metadades, registrar la data, el CRS i la unitat d'observació i identificar els noms físics, els tipus, els dominis i les claus abans d'escriure cap consulta.

La primera fase verifica `municipi_treball`, capa derivada a la micropràctica 2 i heretada físicament dins del GeoPackage `pr3`. Se'n comproven la URI, el recompte d'una entitat, la identitat oficial, el `codi_muni` textual, l'esquema, el CRS, la geometria i l'extensió contra `municipality_icgc_5k`, `municipality_cnig` i el diari. Qualsevol discrepància obliga a aturar el capítol; si l'error prové de `pr2`, no s'edita `dist/pr2-digitalitzacio-cognom.gpkg`, sinó que es regenera aquella instantània i després es torna a crear `pr3` dins d'un candidat complet net. Si el paquet ja s'havia lliurat, aquest candidat es publica com una revisió nova i el paquet anterior es conserva intacte. `municipi_treball` no es recrea ni se sobreescriu per fer coincidir la consulta actual.
...
La tercera família, transport, es perfila amb el mateix criteri abans de qualsevol consulta. A partir dels atributs i les geometries realment disponibles, es defineix un predicat relacionat amb la pregunta del projecte i se'n conserven les entitats candidates a `sandbox/pr3-consultes-cognom.gpkg` com a `transport_candidats_c06`, amb els identificadors d'origen i el criteri exacte. Si la selecció usa `intersects`, les geometries es retenen completes: el retall, les zones d'influència i les superposicions corresponen al capítol 06.

`municipi_treball` és una capa derivada creada a la micropràctica 2; `municipality_icgc_5k` i `municipality_cnig` es mantenen intactes, i `transport_candidats_c06` i les taules de resum o diagnòstic són resultats nous d'aquest capítol. Cap d'aquests noms s'ha d'atribuir com si fos un nom físic dels productes del CNIG. En acabar, es desa el projecte incrustat `pr3` i es crea de nou `sandbox/pr3-consultes-cognom.qgz`, amb camins relatius al GeoPackage homònim. Amb QGIS tancat, la parella es copia a `dist/` sense canviar-ne els noms i es prova des d'una ubicació neta per confirmar que les capes municipals continuen intactes i que les noves sortides, relacions i identificadors estables estan disponibles.
...
| Component | Requisit |
| --- | --- |
| Entrades | `dist/pr2-digitalitzacio-cognom.gpkg`, que es copia a `sandbox/pr3-consultes-cognom.gpkg`; les capes heretades; les edicions descarregades i les metadades de les famílies de límits administratius, *Poblaciones* i transport del CNIG |
| Operacions mínimes | No copiar el `.qgz` de `pr2`; establir un únic projecte incrustat `pr3`; reorientar les fonts locals; verificar `municipi_treball` sense substituir-la; inspeccionar els esquemes reals; executar consultes amb `AND`, `OR`, `NOT` i `NULL`; distingir filtres i selecció; comprovar el criteri municipal; calcular camps derivats en sortides diferenciades; perfilar claus i cardinalitats; preparar un resum abans d'unir una banda 1:N; i diagnosticar una correspondència espacial |
| Resultats | `municipality_icgc_5k`, `municipality_cnig` i `municipi_treball` preservades; camps derivats comprovats en una taula o sortida diferenciada; taula resum amb clau única; registres sense correspondència o amb coincidència múltiple identificats; i `transport_candidats_c06`, tot dins de `pr3-consultes-cognom.gpkg` |
...
| Comprovacions | Exactament un projecte incrustat `pr3`; cap URI local cap a `dist/` o `pr2`; camins relatius al `.qgz`; `municipi_treball` conserva identitat, esquema, CRS, geometria i una entitat; recomptes reconstruïbles de zero/una/múltiples coincidències; absència de col·lisions de clau no resoltes; unicitat de la taula resum; mínim i màxim dels derivats; mostra manual i reobertura de totes les sortides |
| Fitxers que cal conservar | `dist/pr3-consultes-cognom.gpkg`, amb el projecte incrustat `pr3`; `dist/pr3-consultes-cognom.qgz`; i diari amb expressions, recomptes i controls |
:::

La pràctica es considera completa quan les unions es poden tornar a executar a partir de les fonts preparades, la suma dels grups diagnòstics reconstrueix les entrades i els candidats de transport queden disponibles per al capítol 06. `dist/pr3-consultes-cognom.gpkg` es tracta com a fita de només lectura mentre la micropràctica 4 el copia a `sandbox/` amb el nom base `pr4-geoprocessament-cognom`; si es detecta un error, es reconstrueix el candidat complet segons la política de revisions, i un paquet ja lliurat no s'altera. Els registres sense correspondència o amb més d'una candidata formen part del resultat observable i no s'han d'amagar perquè el mapa sembli complet.
